...
- `deploy/`, `docs/`, `demos/`, and `sandpit/` are top-level but are **not** book folders — they're cross-cutting and serve every book:
  ...
  - `demos/` holds four runnable demos (a Django labeling app, a FastAPI+React labeling app, a Huey/SQLite long-jobs app, and a static D3 PSA terminal map) that back specific claims in the `blog/` post `web-dev-for-data-people` and the `web-stack-advisor` skill's worked examples and ladder — see [demos/README.md](demos/README.md). The first three deliberately share one dataset (`demos/sample-data/`) across two of them, an intentional exception to the "stages are self-contained" rule below, explained in that README; the fourth (`demos/psa-terminal-map/`) has its own domain data and sits outside that shared scenario on purpose.
...
- **Explored in this repo (code or notes):** HTML/CSS, JavaScript, jQuery, D3, React, Angular, Flask, Django, FastAPI, Tailwind CSS, and the Python data stack. FastAPI, Django, and Tailwind (CDN browser build + daisyUI, and `@tailwindcss/vite`) are exercised specifically in [`demos/`](demos/README.md) (a runnable comparison, not a curriculum stage) rather than in a book folder — see that folder's README for what each demo proves. `demos/psa-terminal-map/` additionally exercises D3 + static HTML at rung 1 of the `web-stack-advisor` skill's own ladder, and doubles as a runnable showcase of the skill itself (its build trail and live advisor run the skill's own scoring logic, checked by `npm test` against all eight of the skill's documented use cases).
- **In scope but not explored here — no stages will be added:** Streamlit, Dash. Recommend them from general knowledge, and say so when doing it (the repo has no code to point at). Do not create stage folders for them unless the user asks.
...
The tool-choice guidance is also packaged as a reusable, portable skill at [`.claude/skills/web-stack-advisor/`](.claude/skills/web-stack-advisor/SKILL.md) (12 web functionalities, use-case mapping, UI kits, multi-user, background jobs, worked example, plus starter notes for FastAPI/Streamlit/Dash/Tailwind under `references/tools/`). The FastAPI, Django, and Tailwind notes are now backed by `demos/` (actually run in this repo, see `demos/README.md`); the Streamlit and Dash notes are still written from documentation only, not run here. `demos/psa-terminal-map/` goes further than backing individual tool notes — it showcases the skill's method directly: it is built at the rung the skill itself recommends for its use case, its build-trail panel renders the skill's 12-functionality table and ladder as data (`data/advisor-model.json`), and its live advisor runs the skill's actual scoring rules (`js/advisor.js`), checked by a test that every one of the skill's eight documented use cases still produces the stack `references/use-cases.md` documents. Keep the skill and this section consistent, and re-verify its dated ("checked September 2026") facts before relying on them. The skill is meant to be copied into any project, so it must stay self-contained: it may cite `demos/...` paths as evidence that a claim was tested, but always qualified as living in this source repo rather than in the project the skill is installed into (never a bare path implying the folder is present). Blog drafts live under `blog/` and follow [docs/blog-style.md](docs/blog-style.md); the user does not want em-dashes in blog posts.
