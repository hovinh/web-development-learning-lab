...
- `deploy/`, `docs/`, `demos/`, and `sandpit/` are top-level but are **not** book folders — they're cross-cutting and serve every book:
  ...
  - `demos/` holds three runnable demos (a Django labeling app, a FastAPI+React labeling app, and a Huey/SQLite long-jobs app) that back specific claims in the `blog/` post `web-dev-for-data-people` and the `web-stack-advisor` skill's worked examples — see [demos/README.md](demos/README.md). They deliberately share one dataset (`demos/sample-data/`) across two of the three, an intentional exception to the "stages are self-contained" rule below, explained in that README.
...
- **Explored in this repo (code or notes):** HTML/CSS, JavaScript, jQuery, D3, React, Angular, Flask, Django, FastAPI, Tailwind CSS, and the Python data stack. FastAPI and Tailwind are exercised specifically in [`demos/`](demos/README.md) (a runnable comparison, not a curriculum stage) rather than in a book folder — see that folder's README for what each demo proves.
